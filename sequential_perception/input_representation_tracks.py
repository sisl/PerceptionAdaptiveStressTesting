...
class AgentBoxesFromTrackingPost(AgentRepresentation):
    """
    Represents the past sequence of agent states as a three-channel
    image with faded 2d boxes.
    """
    ...
    def draw_agent_boxes(self, center_agent_annotation: Dict[str, Any],
        ...
        nusc = self.nusc_helper.data
        sample_token = center_agent_annotation['sample_token']
        sample = nusc.get('sample', sample_token)
        sensor = 'LIDAR_TOP'
        sd_record = nusc.get('sample_data', sample['data'][sensor])
        ego_record = nusc.get('ego_pose', sd_record['ego_pose_token'])

        ego_annotation = {'sample_token': sample_token,
                        'translation': ego_record['translation'],
                        'size': [1.9, 4.3, 1.0803972657989054],
        ...
        for instance_token, annotations in agent_history.items():
            ...
            for i, annotation in enumerate(annotations):
                ...
                if instance_token != center_agent_annotation['tracking_id'] and np.linalg.norm(np.array(ego_annotation['translation']) - np.array(annotation['translation'])) < 3:
                    continue
        ...
        ego_box = get_track_box(ego_annotation, (agent_x, agent_y), center_agent_pixels, resolution)
        cv2.fillPoly(base_image, pts=[np.int0(ego_box)], color=(60, 168, 13))
...
class StaticLayerFromTrackingBW(StaticLayerRepresentation):
    """
    Creates a representation of the static map layers where
    the map layers are given a color and rasterized onto a
    three channel image.
    ...
    def make_representation(self, instance_token: str, sample_token: str) -> np.ndarray:
        ...
        def color_by_me(agent_yaw_in_radians: float,
                ...
                normalized_rgb_color = [0.5, 0.5, 0.5]
        ...
        lanes = draw_lanes_in_agent_frame(image_side_length_pixels, x, y, yaw, radius=50,
                                          image_resolution=self.resolution, discretization_resolution_meters=0.5,
                                          map_api=self.maps[map_name], color_function=color_by_me)
        ...
        out = image[row_crop, col_crop, :]
        out_gr = np.dot(out[...,:3], [0.2989, 0.5870, 0.1140])
        out_gr = out_gr.astype('uint8')
        stacked_img = np.stack((out_gr,)*3, axis=-1)
        return stacked_img