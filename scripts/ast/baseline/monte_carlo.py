...
def run(args):
    ...
    base_log_dir = '/mnt/hdd/hdelecki/ford_ws/output/mc_small_pred/' + scene_token
    # experiment settings
    ...
    Path(run_experiment_args['log_dir']).mkdir(parents=True, exist_ok=True)

    start = time.time()
    print('Starting MC on scene {}'.format(scene_token))

    n_iterations = 50
    done = False
    fail = False
    for i in range(n_iterations):
        actions_sample = [np.random.randint(9999) for j in range(sim.horizon)]
        sim.reset(0)
        for a in actions_sample:
            print(a)
    ...
    Path(mcts_algo_args['log_dir']).mkdir(parents=True, exist_ok=True)
    if fail:
        summary_path = Path(mcts_algo_args['log_dir']) / 'summary.pkl'
        with open(summary_path, 'wb') as f:
            summary = sim.failure_log
            #json.dump(summary, f)
            pickle.dump(summary, f)

        data_path = Path(mcts_algo_args['log_dir']) / 'data.pkl'
    ...
    # )
    print('Finished MC for scene {}. Took {} sec'.format(scene_token, time.time() - start))
...
if __name__ == '__main__':
    ...
    # Load scenes
    all_scene_tokens = []
    #scene_fname = '/mnt/hdd/hdelecki/ford_ws/src/SequentialPerceptionPipeline/configs/fail_scene_tokens.txt'
    ...
    #scene_idxs = range(250, len(all_scene_tokens))
    scene_idxs = range(len(all_scene_tokens))
    #scene_idxs = range(20)
    #scene_idxs = range(20, len(all_scene_tokens))
